...
public class ItemsViewUtil {
  ...
  public Template fillNewItemView() throws IOException {
    return Template.fromFile("WEB-INF/html/item/new_item.inc.html")
        .withHtml("model_options", genOptionsHtml(AppStoreItem.Model.values(), "model"))
        .withHtml("land_keyboard_options", genOptionsHtml(AppStoreItem.KeyboardLayout.values(), "kb_land"))
        .withHtml("port_keyboard_options", genOptionsHtml(AppStoreItem.KeyboardLayout.values(), "kb_port"))
        .withHtml("character_color", genOptionsHtml(AppStoreItem.CharacterColor.values(), "char_color"));
  }
