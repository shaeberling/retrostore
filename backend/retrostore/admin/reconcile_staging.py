...
def reconcile(
    *,
    ...
    app = _staged_app(matches[0].id, _document_data(matches[0])) if matches else None
    media_documents = _documents(client, "media", "appId", app_id)
    screenshot_documents = _documents(client, "screenshots", "appId", app_id)
    media_by_id = {
        document.id: _staged_media(document.id, _document_data(document))
        for document in media_documents
    }
    screenshots_by_id = {
        document.id: _staged_screenshot(document.id, _document_data(document))
