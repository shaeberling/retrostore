"""Frozen request and response behavior for the nine public RPCs."""
...
from retrostore.api.storage import ApiDataStore, CatalogEntry
...
def build_handlers(storage: ApiDataStore) -> Mapping[str, ApiHandler]:
    """Build the complete handler map consumed by the Flask transport."""

    return RetroStoreApi(storage).handlers


class RetroStoreApi:
    """Public API behavior independent of Flask routing and persistence choice."""

    def __init__(self, storage: ApiDataStore) -> None:
    ...
    def list_apps(self, body: bytes) -> Response:
        params = self._parse_list_apps(body, allow_json_fallback=True)
        response = api_pb.ApiResponseApps()
    ...
    def list_apps_nano(self, body: bytes) -> Response:
        params = self._parse_list_apps(body, allow_json_fallback=False)
    ...
    def fetch_media_images(self, body: bytes) -> Response:
        params = self._parse_media_params(body, allow_json_fallback=True)
        if params is None:
    ...
    def fetch_media_image_refs(self, body: bytes) -> Response:
        params = self._parse_media_params(body, allow_json_fallback=False)
        if params is None:
            return _server_error(_NULL_POINTER_ERROR)
    ...
    def fetch_media_image_region(self, body: bytes) -> Response:
        ...
        slot = next(
            (slot for slot in slots if slot.has_body and slot.image.filename == filename),
            None,
        )
        if slot is None:
            return _server_error(_NULL_POINTER_ERROR)
    ...
    def _parse_get_app(body: bytes) -> str | None:
        ...
        json_value = _parse_json_object(body)
        if json_value is None:
            return None
        value = json_value.get("appId")
        return value if isinstance(value, str) else None

    @staticmethod
    def _parse_list_apps(body: bytes, *, allow_json_fallback: bool) -> api_pb.ListAppsParams | None:
        ...
        try:
            params = api_pb.ListAppsParams(
                start=int(json_value.get("start", 0)),
                num=int(json_value.get("num", 0)),
                query=str(json_value.get("query") or ""),
            )
            trs80_filter = json_value.get("trs80") or {}
            media_names = trs80_filter.get("mediaTypes") or []
    ...
    def _parse_media_params(
        body: bytes, *, allow_json_fallback: bool
