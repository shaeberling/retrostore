"""Storage abstractions used by the public RetroStore API."""
...
@dataclass(frozen=True, slots=True)
class MediaSlot:
    """One API media slot whose body may be loaded only when requested."""
...
class DownloadApp:
    name: str
    media: tuple[DownloadMedia, ...]


class PublicCatalog(Protocol):
    """Additional website routes served from the same catalog."""
...
class ApiDataStore(StateStorage, Protocol):
...
class InMemoryApiDataStore:
    """Deterministic API data store for local and contract tests."""
    ...
    def list_catalog_entries(self) -> Sequence[CatalogEntry]:
        return tuple(
            CatalogEntry(_clone(entry.app), entry.media_types) for entry in self._catalog.values()
        )
    ...
    def save_state(self, state: api_pb.SystemState) -> int:
        with self._state_lock:
            ...
            self._next_state_token = _MIN_STATE_TOKEN if token == _MAX_STATE_TOKEN else token + 1
            return token
