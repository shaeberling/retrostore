...
def write_state_archive(
    ...
    *,
    ...
    objects = {
        _archive_object_path(entry): bytes(entry.body)
...
def load_state_archive(path: Path) -> StateArchive:
    ...
    with zipfile.ZipFile(path) as archive:
        ...
        expected_names = {
            "manifest.json",
            *(
                f"objects/{_archive_object_path(state)}"
...
def state_archive_evidence(value: StateArchive) -> dict[str, Any]:
    return {
        ...
        "earliest_expiry": (
            _format_time(min(state.expires_at for state in value.states))
            if value.states
            else None
        ),
        "latest_expiry": (
            _format_time(max(state.expires_at for state in value.states))
            if value.states
            else None
        ),
    }
...
def _archived_state(
    record: StateTokenRecord, body: bytes, captured_at: datetime
) -> ArchivedState:
