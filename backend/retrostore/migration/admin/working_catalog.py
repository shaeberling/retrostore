...
from retrostore.migration.catalog_mirror.catalog import CatalogMirror, ObjectReader
from retrostore.migration.catalog_mirror.persistence import CatalogSnapshot
...
class WorkingCatalogMaterialization:
    ...
    def counts(self) -> Mapping[str, int]:
        return MappingProxyType({name: len(self.collections[name]) for name in _COLLECTIONS})
...
def working_materialization_to_mirror(
    ...
    expected_manifest = _materialization_sha256(materialization.source, materialization.collections)
...
def build_working_catalog_candidate(
    ...
    baseline_screenshots = {value["id"]: value for value in manifest["screenshots"]}
    ...
    for app_id, document in sorted(staged_apps.items()):
        ...
        if status not in {"STAGING", "DRAFT"}:
            raise ValueError("Publication candidates may include only staged apps or drafts")
        ...
        if is_draft:
            ...
            if document.get("baseSnapshotId") != baseline.source["snapshotId"] or document.get(
        ...
        if is_draft:
            ...
            if any(
                ...
                raise ValueError("Published app draft references another app's screenshot")
        source_author_id = _optional_string(document, "sourceAuthorId") if is_draft else author_id
        app_records.append(
            {
                "id": app_id,
                "name": _string(document, "name"),
                ...
                "first_published_at_ms": (
                    _integer(document, "firstPublishedAtMs") if is_draft else created_at_ms
                ),
        ...
        media_records.append(
            {
                ...
                "filename": _string(document, "filename"),
                "description": _string(document, "description"),
                "upload_time_ms": _timestamp_ms(document.get("createdAt"), "createdAt"),
                **_object_descriptor(document),
            }
        )
    ...
    for screenshot_id, document in sorted(staged["screenshots"].items()):
        ...
        if screenshot_id in baseline_screenshot_ids:
            raise ValueError("Staged screenshot collides with a published screenshot ID")
        _validate_staged_document(document, "screenshot")
        app_id = _required_string(document, "appId")
        if app_id not in staged_apps:
            raise ValueError("Staged screenshot belongs to a non-staged app")
        expected_position = _string_list(staged_apps[app_id], "screenshotIds").index(screenshot_id)
    ...
    combined_apps = [value for value in manifest["apps"] if value["id"] not in replaced_app_ids]
    combined_apps.extend(app_records)
    all_referenced_media = {
        media_id for app in combined_apps for media_id in _working_media_ids(app["media_slots"])
    }
    all_referenced_screenshots = {
        screenshot_id for app in combined_apps for screenshot_id in app["screenshot_ids"]
    }
    combined_media = [
        value
    ...
    return CatalogMirror.from_dict(
        {
            ...
            "source": {
                ...
                "exported_at": datetime.fromtimestamp(latest_update_ms / 1000, tz=UTC)
                .isoformat()
                .replace("+00:00", "Z"),
    ...
    return _sha256(
        {
            ...
            "source": dict(source),
            "collections": {
                name: {key: dict(value) for key, value in sorted(collections[name].items())}
