...
from retrostore.migration.catalog_mirror.persistence import CatalogSnapshot, ImmutableObject
...
class FirestoreCatalogSnapshotStore:
    ...
    def stage(self, snapshot: CatalogSnapshot) -> None:
        ...
        if existing.exists:
            ...
            if existing_data.get("status") == "READY":
                manifest = self._load_snapshot_manifest(snapshot.id, allowed_statuses={"READY"})
                if _manifest_sha256(manifest) != snapshot.manifest_sha256:
                    raise ValueError("Ready Firestore snapshot does not match its manifest digest")
                return
    ...
    def activate(self, snapshot_id: str, manifest_sha256: str) -> None:
        ...
        manifest = self._load_snapshot_manifest(snapshot_id, allowed_statuses={"STAGED", "READY"})
    ...
    def load_active_manifest(self) -> Mapping[str, Any]:
        active = self._client.collection(_CONTROL_COLLECTION).document(_ACTIVE_DOCUMENT).get()
    ...
    def load_snapshot_manifest(self, snapshot_id: str, manifest_sha256: str) -> Mapping[str, Any]:
    ...
    def activate_guarded(
        ...
        *,
        ...
        active_reference = self._client.collection(_CONTROL_COLLECTION).document(_ACTIVE_DOCUMENT)
        ...
        def commit_activation(transaction: Any) -> None:
            ...
            if _manifest_sha256(active_manifest) != expected_active_manifest_sha256:
                raise ValueError("Expected active catalog snapshot changed before activation")
    ...
    def _load_snapshot_manifest(
        ...
        *,
        ...
        return {
            ...
            "screenshots": self._collection_documents(root, "screenshots", transaction=transaction),
...
def validate_migration_identity(project: str, service_account: str) -> None:
    if service_account != migration_service_account(project):
        raise ValueError("Catalog apply must impersonate the dedicated project migration identity")
...
def _require_active_pointer(value: Mapping[str, Any], plan: CatalogActivationPlan) -> None:
    if (
        value.get("snapshot_id") != plan.expected_active_snapshot_id
        or value.get("manifest_sha256") != plan.expected_active_manifest_sha256
    ):
