...
def build_legacy_reverse_plan(
    ...
    *,
    ...
    baseline_author_ids = {
        app.author_id for app in baseline.apps if app.author_id is not None
    }
    ...
    author_allocations = sorted(
        value for value in author_ids if not _is_legacy_long_id(value)
    )
    ...
    media_allocations = sorted(
        media_id
        for media_id in candidate_added_media_ids
        if not _is_legacy_long_id(media_id)
    )
    media_absence_checks = sorted(
        media_id
        for media_id in candidate_added_media_ids
        if _is_legacy_long_id(media_id)
    )
    ...
    plan: dict[str, Any] = {
        ...
        "preconditions": {
            ...
            "legacy_catalog_must_exactly_match_baseline_snapshot": baseline_evidence[
                "snapshot_id"
            ],
        ...
        "blockers_before_apply_can_exist": [
            ...
            *(
                ["legacy_author_id_allocation_map_required"]
                if author_allocations
                else []
            ),
            *(
                ["legacy_media_id_allocation_map_required"]
                if media_allocations
                else []
            ),
            *(
                ["legacy_screenshot_blob_writer_required"]
                if screenshot_writes
                else []
            ),
            *(
                ["explicit_destructive_removal_confirmation_required"]
                if removal_count
                else []
            ),
        ],
