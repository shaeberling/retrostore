...
import retrostore.admin.apps as apps
...
def _rpk() -> ValidatedRpk:
    return ValidatedRpk(
        ...
        media=(
            RpkMedia(
                "disk-1",
                validate_media_upload(filename="disk_0.dmk", body=b"disk", description=""),
            ),
            RpkMedia(
                "command",
                validate_media_upload(filename="command.cmd", body=b"command", description=""),
            ),
        ),
        screenshots=(
...
def test_app_record_form_normalizes_valid_input() -> None:
    result = apps.validate_app_form(_form())

    assert result.errors == {}
    assert result.app_input == _app_input()
    assert result.values["request_id"] == "11111111-1111-4111-8111-111111111111"


def test_app_record_form_rejects_invalid_fields_and_preserves_values() -> None:
    result = apps.validate_app_form(
        _form(
            request_id="bad",
            name=" ",
            version="v" * 65,
    ...
    assert result.app_input is None
...
class FakeFirestore:
    def __init__(self, *, apps=(), authors=(), media=(), screenshots=()):
        ...
        self.collections = {
            "apps": FakeCollection(
                {snapshot.id: FakeDocument(snapshot.id, snapshot) for snapshot in apps}
            ),
            "authors": FakeCollection(
                {snapshot.id: FakeDocument(snapshot.id, snapshot) for snapshot in authors}
            ),
            "media": FakeCollection(
                {snapshot.id: FakeDocument(snapshot.id, snapshot) for snapshot in media}
            ),
            "screenshots": FakeCollection(
                {snapshot.id: FakeDocument(snapshot.id, snapshot) for snapshot in screenshots}
            ),
...
def test_publishers_list_only_owned_app_records() -> None:
    ...
    catalog = apps.FirestoreAppRepository(client)
...
def test_materialized_baseline_supports_legacy_ids_and_object_paths() -> None:
    ...
    detail = apps.FirestoreAppRepository(client).get_app_detail(ADMIN, app_id)
...
def test_draft_creation_writes_author_app_and_audit_atomically(monkeypatch) -> None:
    monkeypatch.setattr(apps.firestore, "transactional", lambda function: function)
    client = FakeFirestore()
    catalog = apps.FirestoreAppRepository(client)
    request_id = "22222222-2222-4222-8222-222222222222"

    app = catalog.create_app(identity=PUBLISHER, request_id=request_id, app_input=_app_input())
...
def test_rpk_import_uploads_immutable_assets_and_commits_metadata_atomically(
    ...
    monkeypatch.setattr(apps, "new_asset_id", lambda: next(asset_ids))
    client = FakeFirestore()
    store = FakeObjectStore()
    catalog = apps.FirestoreAppRepository(client, store)
...
def test_rpk_import_refuses_existing_app_id_and_removes_new_objects(monkeypatch) -> None:
    ...
    monkeypatch.setattr(apps, "new_asset_id", lambda: next(asset_ids))
    client = FakeFirestore(apps=(FakeSnapshot("8c028afe-96b3-11e7-a68b-5b6133ca5f0c", {}),))
    store = FakeObjectStore()
    catalog = apps.FirestoreAppRepository(client, store)

    with pytest.raises(apps.AppConflictError, match="already uses"):
...
def test_draft_creation_is_idempotent_for_same_request_and_content(monkeypatch) -> None:
    monkeypatch.setattr(apps.firestore, "transactional", lambda function: function)
    ...
    app = apps.AppRecord(
        ...
        model="MODEL_I",
        category="GAME",
        author_id=apps._author_id("Jane Doe"),
        author_name="Jane Doe",
        publisher_uid="publisher-1",
        publisher_email="publisher@example.test",
        revision=1,
    )
    existing = apps._app_document(app, request_id=request_id)
...
def test_draft_creation_rejects_reused_request_id(monkeypatch) -> None:
    monkeypatch.setattr(apps.firestore, "transactional", lambda function: function)
    request_id = "44444444-4444-4444-8444-444444444444"
    client = FakeFirestore(apps=(FakeSnapshot(request_id, {"creationRequestId": request_id}),))
    catalog = apps.FirestoreAppRepository(client)

    with pytest.raises(apps.AppConflictError, match="already in use"):
        catalog.create_app(
            identity=PUBLISHER,
            request_id=request_id,
            app_input=_app_input(),
...
def test_draft_update_checks_ownership_revision_and_audits_atomically(
    monkeypatch,
) -> None:
    monkeypatch.setattr(apps.firestore, "transactional", lambda function: function)
    ...
    updated = catalog.update_app(
        ...
        app_id=app_id,
        expected_revision=3,
        app_input=_app_input(name="New Name", author_name="New Author"),
    )
...
def test_draft_update_rejects_non_owner_and_stale_revision(monkeypatch) -> None:
    monkeypatch.setattr(apps.firestore, "transactional", lambda function: function)
    ...
    with pytest.raises(apps.AppAuthorizationError):
        catalog.update_app(
            identity=PUBLISHER,
            app_id=app_id,
            expected_revision=2,
    ...
    with pytest.raises(apps.AppConflictError, match="concurrently"):
        catalog.update_app(
            identity=ADMIN,
            app_id=app_id,
            expected_revision=1,
...
def test_published_app_allows_direct_administrator_update(monkeypatch) -> None:
    ...
    updated = catalog.update_app(
        ...
        app_id=app_id,
        expected_revision=1,
        app_input=_app_input(name="Updated Published Game"),
    )
...
def test_publisher_cannot_mutate_a_published_app(monkeypatch) -> None:
    ...
    with pytest.raises(apps.AppReadOnlyError, match="administrators"):
        apps.FirestoreAppRepository(client).update_app(
            identity=PUBLISHER,
            app_id=app_id,
            expected_revision=1,
            app_input=_app_input(),
...
def test_publish_app_atomically_flips_status_without_building_a_snapshot(
    ...
    monkeypatch.setattr(apps.firestore, "transactional", lambda function: function)
    monkeypatch.setattr(apps.time, "time_ns", lambda: 1_234_000_000)
    ...
    published = apps.FirestoreAppRepository(client).publish_app(
        identity=PUBLISHER,
        app_id=app_id,
        expected_revision=2,
    )
...
def test_draft_delete_requires_current_revision_and_keeps_author(monkeypatch) -> None:
    monkeypatch.setattr(apps.firestore, "transactional", lambda function: function)
    app_id = "77777777-7777-4777-8777-777777777777"
    client = FakeFirestore(
        apps=(
...
def test_generated_request_ids_are_normalized_uuid4() -> None:
    value = apps.new_app_request_id()

    assert str(uuid.UUID(value)) == value
    assert uuid.UUID(value).version == 4


def test_media_record_upload_replaces_slot_atomically_and_cleans_old_object(
    monkeypatch,
) -> None:
    monkeypatch.setattr(apps.firestore, "transactional", lambda function: function)
    ...
    monkeypatch.setattr(apps, "new_asset_id", lambda: new_id)
    old_sha = "0" * 64
    ...
    catalog = apps.FirestoreAppRepository(client, store)
    upload = validate_media_upload(filename="disk.dmk", body=b"new disk", description="Boot disk")
    ...
    assert client.transaction_value.creates[1][1]["eventType"] == ("STAGED_MEDIA_UPLOADED")
    assert client.transaction_value.sets[0][1]["mediaSlots"]["disks"][0] == new_id


def test_screenshot_record_upload_and_order_move_are_revision_guarded(
    monkeypatch,
) -> None:
    monkeypatch.setattr(apps.firestore, "transactional", lambda function: function)
    ...
    monkeypatch.setattr(apps, "new_asset_id", lambda: second_id)
    app_document = {
    ...
    catalog = apps.FirestoreAppRepository(client, store)
    upload = validate_screenshot_upload(filename="screen.png", body=b"\x89PNG\r\n\x1a\ncontent")
    ...
    assert client.transaction_value.creates[1][1]["eventType"] == ("STAGED_SCREENSHOT_UPLOADED")
    ...
    move_client = FakeFirestore(apps=(FakeSnapshot(app_id, move_app),), screenshots=screenshots)
    move_catalog = apps.FirestoreAppRepository(move_client, store)
    ...
    assert move_client.transaction_value.creates[0][1]["eventType"] == ("STAGED_SCREENSHOT_MOVED")


def test_app_record_detail_and_delete_cascade_verify_asset_relationships(
    monkeypatch,
) -> None:
    monkeypatch.setattr(apps.firestore, "transactional", lambda function: function)
