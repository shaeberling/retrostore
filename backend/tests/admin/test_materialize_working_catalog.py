...
from retrostore.admin import materialize_working_catalog
from retrostore.admin.working_google_cloud import WorkingCatalogMaterializationReport


def test_dry_run_builds_the_working_plan_without_cloud_clients(
    tmp_path, monkeypatch
) -> None:
    from tests.admin.test_working_catalog import _mirror
...
def test_apply_requires_exact_confirmation_and_migration_identity(
    tmp_path, monkeypatch
) -> None:
    from tests.admin.test_working_catalog import _mirror
...
def test_apply_serializes_mapping_proxy_report(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        materialize_working_catalog.firestore, "Client", lambda **kwargs: object()
    )
