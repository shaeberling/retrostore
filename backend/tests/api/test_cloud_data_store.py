...
from retrostore.catalog import FirestoreCatalogRepository
...
def test_repository_reads_only_published_top_level_documents() -> None:
    ...
    repository = FirestoreCatalogRepository(client)
...
def test_media_references_and_regions_do_not_download_unneeded_objects() -> None:
    repository = FirestoreCatalogRepository(FakeFirestore(_documents()))
...
class FakeReference:
    ...
    def get(self):
        return FakeSnapshot(self.id, self.client.documents.get(self.collection, {}).get(self.id))
