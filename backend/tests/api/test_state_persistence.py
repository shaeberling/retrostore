...
class MemoryTokenStore:
    ...
    def claim(
        ...
        *,
        ...
        replaced = (
            existing.payload if existing is not None and existing.expires_at <= created_at else None
        )
    ...
    def list_live(self, *, now: datetime) -> tuple[StateTokenRecord, ...]:
        return tuple(
            record for _, record in sorted(self.records.items()) if record.expires_at > now
        )
