...
from retrostore.api_compat.plan_legacy_state_reverse_sync import main
from retrostore.api_compat.state_archive import write_state_archive
...
def test_plan_is_token_free_deterministic_and_has_atomic_route_preconditions(
    ...
    body = api_pb.SystemState(model=api_pb.MODEL_I).SerializeToString(
        deterministic=True
    )
    ...
    assert main(
        ["--state-archive", str(archive_path), "--output", str(output)]
    ) == 0
