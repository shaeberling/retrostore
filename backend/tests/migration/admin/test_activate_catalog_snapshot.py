...
from retrostore.migration.admin import activate_catalog_snapshot
...
def test_catalog_activation_is_a_validating_zero_write_dry_run(tmp_path, monkeypatch) -> None:
...
def test_catalog_activation_requires_four_exact_apply_confirmations(tmp_path, monkeypatch) -> None:
    store = _configure(monkeypatch)
...
def test_catalog_activation_applies_only_after_exact_confirmation(tmp_path, monkeypatch) -> None:
    store = _configure(monkeypatch)
    arguments = [
        *_arguments(tmp_path),
        "--apply",
