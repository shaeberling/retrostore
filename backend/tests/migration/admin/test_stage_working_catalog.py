...
from retrostore.migration.admin import stage_working_catalog
from retrostore.migration.admin.working_catalog import build_working_catalog_materialization
from retrostore.migration.catalog_mirror import MappingObjectReader, build_catalog_snapshot
from tests.migration.admin.test_working_catalog import _mirror, _staged_changes
from tests.migration.catalog_mirror.test_catalog import _reader
...
def _configure(monkeypatch, *, with_changes=False):
    mirror = _mirror()
    materialization = build_working_catalog_materialization(mirror, build_catalog_snapshot(mirror))
    ...
    monkeypatch.setattr(stage_working_catalog.firestore, "Client", lambda **kwargs: object())
    ...
    monkeypatch.setattr(stage_working_catalog, "FirestoreWorkingCatalogStore", FakeWorkingStore)
    monkeypatch.setattr(
        stage_working_catalog,
...
def test_apply_reconciles_the_ready_snapshot_without_activation(tmp_path, monkeypatch) -> None:
    snapshots = _configure(monkeypatch)
...
def test_apply_requires_confirmation_before_cloud_credentials(tmp_path, monkeypatch) -> None:
    monkeypatch.setattr(
        stage_working_catalog,
        "gcloud_impersonated_credentials",
