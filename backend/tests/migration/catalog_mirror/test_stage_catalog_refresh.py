...
import retrostore.migration.catalog_mirror.stage_catalog_refresh as refresh_command
from retrostore.migration.catalog_mirror import build_catalog_snapshot, import_catalog_mirror
...
def test_dry_run_reports_id_only_changes_without_cloud_clients(
    ...
    assert (
        refresh_command.main([*_arguments(candidate, output), "--baseline-archive", str(baseline)])
        == 0
    )
