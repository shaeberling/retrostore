...
from retrostore.mirror.plan_legacy_reverse_sync import (
...
def test_cli_builds_a_deterministic_value_free_reverse_plan(tmp_path: Path) -> None:
    ...
    assert plan["blockers_before_apply_can_exist"] == [
...
def test_plan_identifies_legacy_numeric_author_allocation_requirement(
    ...
    assert plan["legacy_requirements"]["author_ids_requiring_allocation"] == [
        "author-uuid"
    ]
    assert "legacy_author_id_allocation_map_required" in plan[
